...
该方法通过选择输入特征的子集来形成每个训练集。子集可以随机选择，也可以根据领域专家的建议选择。相关的研究表明，对那些含有大量冗余特征的数据集，该方法的性能非常好。随机森林（Random Forest）就是采用处理输入特征的集成学习方法，它用决策树作为基分类器。
...
1〉Bagging方法得到的集成分类器几乎总是比单个分类器效果要好；

2〉Boosting方法得到的集成分类器的效果可能明显优于Bagging方法得到的分类器和单个分类器；

3〉然而对于某些数据集Boosting的结果有可能比单个分类器的性能还低；

4〉使用随机化人工神经网络的初始权值来进行集成的方法往往能够取得和Bagging同样好的效果；

5〉Boosting方法的效果至少有一定程度依赖于数据集，而Bagging方法对数据集的依赖却没有那么明显。
